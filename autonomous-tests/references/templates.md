...
**Rules**: Number fixes (`## Fix 1:`, `## Fix 2:`). Metadata summary table per fix. `### Problem` / `### Evidence` / `### Suggested Resolution` subsections. Fenced code blocks for evidence. Reference test IDs for traceability. `---` between fixes. Never expose credentials.
